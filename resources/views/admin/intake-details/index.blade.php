...
<div class="data-section">
    <div class="section-header">
        <h3 class="section-title">
            <i class="fas fa-calendar-alt"></i> All Intake Details
        </h3>
        <a href="{{ route('admin.intake-details.create') }}" class="btn-add" title="Add New Intake Detail">
    ...
    <div class="table-responsive">
        <table class="data-table">
            ...
            <tbody>
                @forelse($intakeDetails as $detail)
                    <tr>
                        <td>{{ $detail->id }}</td>
                        <td>{{ $detail->intake->name ?? 'N/A' }}</td>
                        <td>{{ $detail->event_name }}</td>
                        <td>{{ $detail->start_date ? \Carbon\Carbon::parse($detail->start_date)->format('M d, Y') : 'N/A' }}</td>
                        <td>{{ $detail->end_date ? \Carbon\Carbon::parse($detail->end_date)->format('M d, Y') : 'N/A' }}</td>
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('admin.intake-details.show', $detail) }}" class="action-btn view" title="View Details">
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="empty-state">
                                ...
                                <h5>No Intake Details Found</h5>
                                <p>Get started by adding your first intake detail to the system.</p>
                                <a href="{{ route('admin.intake-details.create') }}" class="btn btn-primary">
                                    <i class="fas fa-plus me-2"></i>Add Intake Detail
                                </a>
                            </div>
                        </td>
                    </tr>
        ...
        @if(method_exists($intakeDetails, 'links'))
            <div class="pagination-wrapper">
                {{ $intakeDetails->links('pagination::bootstrap-5') }}
            </div>
        @endif
    </div>
</div>
